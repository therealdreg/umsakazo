...
# TRL Calibration Fixture Board by Mario Hellmich

Mario Hellmich designed this board to carry every standard needed for a full TRL (thru-reflect-line) calibration, plus a few extra verification standards, together with two test fixtures for measuring 0805 SMD components in series-thru and shunt-thru configurations.

![](stuff/images/tricalfix.jpg)

(Photo by Mario Hellmich, reproduced from his project page.)

https://www.mariohellmich.de/projects/trl-cal/trl-cal.html

Physically it is a plain FR4 board with a ground plane on the back, designed around a nominal 1.55 mm thickness and 35 µm copper, ordered from JLCPCB's standard prototype service with no impedance control, and laid out in KiCad. Every trace on it is a grounded, fenced coplanar waveguide with a 1 mm center conductor and 0.2 mm gaps to the coplanar ground on each side, sized for 50 Ω with the usual closed-form coplanar-waveguide formulas assuming a relative permittivity of 4.5 (his design notes also list a loss tangent of 0.02). Those figures, and every other number given here for the board itself, come from his page, from the design notes printed on his schematic, and from the KiCad and Gerber files he publishes alongside them; what is said below about the layout is read from those files and from his photo, the connector figures come from the manufacturer's application note, and anything derived or suggested by this document is labelled as such. Twelve Cinch edge-launch SMA jacks feed the twelve ports, and they are by far the most expensive thing on the board: at about 7.35 € each, Hellmich notes, they "essentially determine the cost of this amusement." The silkscreen states the design goal: 120 MHz to 2 GHz, for a line phase shift of 10° to 170° across that range, a looser window than the 20° to 160° rule of thumb his own page quotes (he does not comment on the difference); both windows are discussed below.

Several of this board's layout choices are worth examining individually, since they illustrate general principles of good RF fixture design.

TRL calibration itself needs only three standards: a thru, a reflect (here, the Short), and a line, a transmission-line segment 45 mm longer than the thru between the reference planes (76 mm end to end), whose phase shift relative to the thru must stay inside a usable window at every frequency in the calibration band. At exactly 0° or 180° the line becomes indistinguishable from the thru and the calibration equations cannot be solved; too close to either limit, noise takes over. The usual rule of thumb is 20° to 160°, under which a single line covers only about an 8:1 span of frequency; Hellmich's silkscreen accepts 10° to 170°, which stretches his single line to 17:1 at the price of more noise sensitivity near both ends of the band, and wider bands than that are split across several lines of different lengths. The reflect does not need to be known exactly: its magnitude should be close to 1, it must be identical at both ports, and its phase only has to be known roughly, enough to tell a short from an open, because the equations leave a sign ambiguity that only that rough knowledge resolves. The thru is assumed to be flush: zero length, no loss and no reflection. What settles the name is that assumption rather than the copper, and it has to be, because on a board there is nothing to mate flush: this board's Thru is itself 31 mm of the same coplanar line, and only the zero-length assumption keeps the scheme a TRL. Enter the thru's own length into the cal-kit definition instead, so that the reference planes stay at its two ends rather than collapsing onto its center, and the same three standards are usually called line-reflect-line. And there is one consequence of TRL that is easy to miss: in an open-short-load-thru calibration the load is what defines the reference impedance, but in TRL that job falls to the line, so everything a TRL calibration reports is referenced to whatever impedance the line really has, not to a nominal 50 Ω. Open and Mismatch take no part in that core solution; they are on the board for verification. Match takes no part in it either, but has a job of its own, described below.

All four one-port standards, Match, Open, Short and Mismatch, sit 15.5 mm inward from the board edge that carries their SMA jack, by the designer's own figures, while the Thru measures twice that, 31 mm end to end (Hellmich states the 15.5 mm offset directly; the 31 mm follows from doubling it, and also from his 76 mm Line being exactly 45 mm longer than the Thru). Because the Thru is assumed to have zero length, the calibration collapses its two reference planes onto a single point at the Thru's physical center, and because the Thru is built from two of those 15.5 mm sections back to back, that center lands precisely on the same 15.5 mm mark where every one-port standard sits, so each of them is meant to have zero offset from the reference plane the Thru defines, with calibration and verification standards alike drawn to the same 15.5 mm mark relative to their own connector. His Gerbers show that working out differently for each kind of standard, as it must. The Short is the literal case: its coplanar corridor ends exactly on the mark, where the ground fill closes in on the trace. The Match and Mismatch attach their resistors starting on the mark, with the connector-facing edge of the pads, and the copper then continues past it to 16.9 mm from the board edge. That extra millimetre and a half is parasitic rather than offset: an offset is line between the plane and the element, whereas this is pad hanging beyond the junction where the shunt already attaches, and pad wider than the trace at that. The Open is the one whose plane, rather than merely its copper, ends up past the mark, because for an open the plane is wherever the conductor stops. KiCad's microwave-stub tool finishes its center conductor with a pair of 1 mm square pads whose far edge lands at 16.0 mm instead of 15.5 mm, so the Open's plane sits about half a millimetre out, roughly 4° of extra round-trip phase at 2 GHz. It is a verification standard that the TRL solution never touches, so nothing in his calibration depends on it. Each standard is then built at that mark in the simplest way the coplanar geometry allows. The Match is two 100 Ω resistors from the end of the center conductor, one to the coplanar ground on each side, giving 50 Ω with a symmetric current return; the Mismatch is the same arrangement with two 50 Ω resistors, giving a nominal 25 Ω verification standard whose reflection coefficient against a 50 Ω reference is −1/3, an |S11| of about −9.5 dB (this document's arithmetic, not a measurement he reports; against the roughly 46.6 Ω that this board's Line turned out to have, and that its TRL calibration therefore takes as its reference, as described below, it would be closer to −0.30, about −10.4 dB). The Short is simply the center conductor running straight into the surrounding top-layer ground copper, a short that, unlike a microstrip short, needs no via down to the backside plane, because the ground is right there on the same layer; the via fence simply wraps around the end of the trace, with one via sitting on the trace axis exactly one fence pitch past the junction, so the backside plane is tied in there as well. The Open is simply the center conductor stopping. Match, for its part, can also substitute for the line at frequencies too low for a practical line length, because a perfect match looks like a line of infinite length. That variant of TRL is called thru-reflect-match, or TRM, and Hellmich uses exactly that to extend his calibration downward. Giving all four one-port standards the same 15.5 mm of trace regardless of role is what gives them that zero offset, whether the core calibration uses the standard or not (Hellmich notes the reflect would not even need it).

The series-thru and shunt-thru test fixtures build on the same idea, but applied differently. Rather than putting a single reference plane on top of the component, each fixture's two reference planes sit at the outer edges of the 0805 pads themselves, consistent with the same 15.5 mm convention used everywhere else on the board. That alone is enough to give each fixture an overall SMA-to-SMA length close to the Thru's, without the total length needing to match it exactly. It also has a direct consequence: calibration removes everything up to those two pad edges, but not what lies between them, so the pads and the component together are always part of what the measurement reports as "the DUT." Placing every reference plane consistently makes the board self-consistent; it does not make the pads or their immediate surroundings disappear from the result. The two fixtures exist because they suit different impedance ranges, as Hellmich explains, citing Keysight's application notes. The series-thru arrangement, where the part sits in the signal path and the impedance comes out of S21 as Z = 2·Z0·(1 − S21)/S21, is best for high impedances (the kΩ range and up, and above roughly 100 MHz); the shunt-thru arrangement, where the part hangs from the line to ground and Z = (Z0/2)·S21/(1 − S21), is the RF equivalent of a Kelvin connection and is best for very low impedances, below 100 mΩ; and the ordinary one-port S11 measurement used throughout this document, Z = Z0·(1 + S11)/(1 − S11), is best in between, roughly 1 Ω to 10 kΩ and up to a couple of hundred MHz. In all three formulas Z0 is the reference impedance of the calibration. That is the same family of advice as the stay-near-50 Ω lesson at the start of this document, with one distinction worth keeping straight: Hellmich's ranges say which of the three configurations gives the best accuracy where, whereas the 5 to 500 Ω zone given earlier says how far a single one-port reading on a NanoVNA can be trusted; the two S21 arrangements are what take over at the extremes where a one-port measurement runs out of resolution.

Every trace on the board, standards and fixtures alike, also carries a row of ground vias running along each side. This is the "fenced" half of "grounded, fenced coplanar waveguide" (the backside ground plane is the "grounded" half): the vias stitch the coplanar ground fill on each side of the signal trace down to the backside ground plane at frequent, regular intervals. That shared connection keeps the top-side ground and the backside ground at the same potential, and it likewise ties the two coplanar ground strips flanking the trace to each other through that plane. Doing this suppresses two parasitic modes along the line within the board's intended operating range: the parallel-plate mode between the two ground planes, and the slotline-like odd mode between the two coplanar ground strips. Left unsuppressed, either one can siphon energy out of the signal path and add spurious resonances to the measurement. The connector manufacturer's own application note asks for exactly this, ground vias on both sides of the coplanar line along its entire length, spaced about 1.3 mm apart; his files put them at a uniform 1.25 mm pitch, 1.25 mm out from the trace axis on either side.

The choice of SMA connector matters just as much. Hellmich's twelve jacks are edge-launch types: the center pin lands directly on the top-layer trace right at the board's cut edge, without ever needing a drilled via to cross the board thickness. A vertical, through-hole SMA jack, by contrast, brings its pin straight down through a plated hole and meets the trace at the top of that hole, a 90° turn in the signal path that adds series inductance and a local impedance discontinuity, with a short stub of pin left protruding beyond the joint, effects that become more significant as the wavelength shrinks toward the size of the discontinuity. Edge-launch mounting avoids that specific problem, though it is not perfectly free of its own. The launch geometry the manufacturer publishes for these connectors is sized for thin microwave laminates, around 0.4 mm of low-loss substrate over a ground plane, where its 50 Ω coplanar line is only about 0.7 mm wide with 0.25 mm gaps; on 1.55 mm of FR4 the backside ground is further away and contributes less, so reaching the same 50 Ω with the 0.2 mm gaps Hellmich chose takes a wider center conductor, his 1 mm. Rather than narrow the trace down to the recommended footprint where it meets the connector, and so create a step in the center conductor's width right at the launch, he runs the full 1 mm width straight through the launcher pad. The trace is therefore a little wider than the connector was designed for, a compromise he judged to add little error below 2 GHz precisely because there is no discontinuity in the center conductor width. The edge-launch choice also explains the board's stepped outline: an edge-launch trace has to end at a board edge, so each two-port element sets the width of the tier it sits on, 31 mm for the Thru, 76 mm for the Line (the one-port standards, needing only 15.5 mm each, are paired up facing each other inside that 76 mm body), and for the two fixtures 31 mm plus whatever lies between their pad edges: his board outline gives 33.83 mm for the series fixture, whose two 0805 pads sit end to end between the 15.5 mm marks, and 32.4 mm for the shunt fixture, where the two reference planes are a single 1.4 mm pad width apart, which is why the photo shows one more small step in the right-hand edge between J10 and J12.

![](stuff/images/conex.png)

(Detail of connector J9 on Hellmich's board: the red ring marks where the launcher's center pin lands directly on the top-layer trace at the cut edge of the board.)

A related choice is the surface finish, and it is the opposite of what the connector's application note recommends. That note asks for ENIG on the high-frequency conductors and no solder mask over the coplanar trace or its gaps; Hellmich deliberately keeps the solder mask on the center conductors and specifies the trace surface as plain copper under it. His reasoning is that the finishes a standard pool service offers are, in general, not well suited to a line meant to work up to 2 GHz, and he gives two examples: hot-air solder levelling (HASL) leaves a poorly defined surface geometry, and ENIG puts only 50 to 100 nm of gold over about 5 µm of nickel, so at RF the skin effect forces a large fraction of the current into the nickel, a worse conductor than copper and one whose electroless deposition leaves it with phosphorus and with unpredictable, frequency-dependent magnetic properties. Solder mask has its own poorly defined thickness and permittivity, but the finishes a pool service applies, HASL and ENIG alike, go on after the mask and only onto the copper it leaves exposed, so keeping the mask on the trace is what guarantees the conductor underneath stays bare copper. This is the same family of nickel-and-skin-effect argument made earlier in this document, with one refinement: the earlier passage treats the nickel simply as ferromagnetic, while Hellmich's point is that the magnetic behaviour of electroless nickel depends on its phosphorus content and is therefore unpredictable. Both boards are FR4, so the two conclusions differ by frequency and by purpose rather than by substrate: on a learning board whose RF paths are a few centimetres long, the extra conductor loss a nickel skin adds amounts to a small fraction of a dB and is a non-issue next to the parasitics of the parts themselves; on a set of calibration standards whose behaviour has to be predictable and repeatable up to 2 GHz, Hellmich wanted a conductor whose properties he could count on. And, one might add, copper under solder mask costs nothing extra from a pool service, so the precaution was free.

Right around the footprint of the SMD component in the series fixture, the designer has pulled the ground copper fill back, leaving a small cleared rectangle instead of letting it close in on the pads at the usual 0.2 mm gap. Without that clearance, the ground fill would run at its usual 0.2 mm spacing along the whole edge of each pad, and in his files those pads are 1.4 mm across where the trace is 1 mm, so each pad would present more copper to a ground that stayed just as close. The fringing field between pad and ground is then a parasitic shunt capacitance sitting exactly where the component sits, on top of whatever the component itself contributes, and it also drags the local impedance of the line below 50 Ω. Pulling the ground back therefore does two jobs at once: it reduces that capacitance, though it cannot remove it, and the local rise in impedance it brings pushes back against the dip the wider pad would otherwise cause. What it cannot touch at all is the pad's capacitance to the backside ground plane through 1.55 mm of FR4, which is small and stays. In the layout that cleared rectangle measures about 3.2 by 3.4 mm, and it pushes the via fence from its usual 1.25 mm off the trace axis out to 2.5 mm. The shunt fixture does without that pullback, and his layout has no cleared zone there at all: its signal-side pad sits inside the normal trace width with the usual gaps running straight past it, and the other pad has to sit in the ground fill by definition, where he has carried the same 1.25 mm fence grid around it, so the component's ground reaches the backside plane through several vias close by. A related clearance sits at the end of the Open trace, though it does a different job: there the cleared strip measures 2.5 by 1.4 mm, exactly the trace plus its two gaps, so the fill still runs alongside the conductor at the usual 0.2 mm and is only held back in front of the open end. The fill could never short the stub there, because the 0.2 mm clearance rule keeps it off the copper everywhere; what it would do is wrap round the tip at that same distance and load the open with the one parasitic an open standard wants kept small, its end fringing capacitance. The strip starts on the 15.5 mm mark itself, half a millimetre short of the copper tip, so 2 mm of its 2.5 mm length lies in front of the open end, and that is what limits the capacitance. Around the inner pads of the Match and Mismatch resistors the pullback is the sideways kind again, a cleared rectangle 3.1 by 3.6 mm covering the inner half of each resistor and reaching past them (their outer pads sit in the ground fill, as they must). The Open's strip and the Match and Mismatch rectangles all show in the full-board photo at the top of this section as darker patches where the top copper is gone. Clearing the ground away from a pad is a small habit, but it is exactly this kind of detail that keeps a fixture's own parasitics small enough not to swamp the component they are always lumped in with.

![](stuff/images/closecomp.png)

(Detail of the series fixture: the red outlines mark the ground copper cleared away above and below the 0805 pads, one rectangle around the whole footprint whose middle is hidden by the component itself.)

Hellmich then measured what the board actually does, on a professional analyzer (he does not name it; the screenshots on his page carry the Rohde & Schwarz logo, and since its firmware performed TRL it must, by that four-receiver argument of his, quoted below, be a four-receiver instrument), and his results are the best argument for everything above. He first measured the impedance of the coplanar line itself, with a coaxial calibration that leaves the reference planes in the SMA connectors, and got consistent answers in several ways: 46.6 Ω from a curve fit to the input impedance of the 76 mm Line with its far end shorted by an SMA short from a cal kit, swept against frequency; 47.8 Ω and 46.0 Ω read off that same shorted-line sweep at half the frequency of its first impedance peak and at half the frequency of its second (his markers M1 and M3, at 212 MHz and 640 MHz, half of the 424 MHz and 1.28 GHz peaks, which themselves read about 2.1 kΩ and 1 kΩ); and 47.4 Ω from the minimum-maximum method, the geometric mean of the first minimum and the first maximum of the input impedance seen with the far end terminated in an SMA 50 Ω load. Those halved frequencies are not arbitrary: the peaks are where the shorted line is a quarter and three quarters of a wavelength long, so half of each puts it at an eighth and three eighths, and there the magnitude of a shorted line's input impedance equals its characteristic impedance. A fourth, textbook method, the geometric mean of the open- and short-terminated input impedances, he also tried and found impractical on real data: an open always shows some fringing, so the poles and zeros of the two sweeps do not coincide exactly, and neither termination reflects exactly ±1, which leaves a spiky curve full of unphysical jumps. Those four numbers, from the three methods that gave one, lie between about 4% and 8% below the 50 Ω the formulas were aimed at, the fitted value about 7% below, a deviation he considers to be within the limits to be expected from a board process with no impedance control, while noting that the figure was measured through the connectors and so also carries the errors of the launch transitions and of that SMA short. As already said, a TRL calibration is referenced to its line, so his TRL-corrected results are referenced to that measured 46.6 Ω or so rather than to 50 Ω, wherever the Line rather than the Match of his TRM extension is the standard in use. The 45 mm of line between the two reference planes also reached 180° of phase shift at 1.865 GHz instead of just above 2 GHz, so the board misses its own design goal, and he caps his sweeps at 1.8 GHz. Scaling from that 1.865 GHz figure with the 20° to 160° rule of thumb gives a comfortable band of roughly 200 MHz to 1.66 GHz (this document's arithmetic, not his); his ferrite-bead sweeps run from 50 MHz to 1.8 GHz, past both ends of that window, where by the same arithmetic the Line offers only about 5° at the bottom and about 174° at the top, which is presumably why his resonance measurement below, swept from 30 MHz, used the TRM extension. He then soldered a 60 Ω 0805 ferrite bead (TDK MMZ2012R600AT000) into the series fixture and measured it twice: once with a TRL calibration that puts the reference planes at the pad edges, and once with a coaxial calibration against his own cheap SMA cal kit that leaves them in the SMA connectors. The magnitude of S21 barely changed, a fraction of a dB, but the phase changed a great deal, and so did the impedance calculated from S21; in his words, "even a few mm of trace matter a lot, and can make a circuit look much different impedance-wise." A 47 nH inductor stacked on a 220 pF C0G capacitor, measured in both fixtures with TRL plus the TRM extension, resonated at 49.96 MHz in the series fixture against 49.49 MHz from the nominal values, and gave the same impedance curve from either fixture despite very different-looking S21 traces. His own conclusion is that the board is a genuinely useful lab tool, that more line standards and a multi-line TRL would extend its upper limit, and that FR4 becomes increasingly uncontrollable as frequency rises, so precise work belongs on an impedance-controlled process.

And it is worth restating the point this board makes indirectly, and in fact reinforces through its own reference-plane placement and through that ferrite-bead experiment: whatever impedance you measure for an SMD component soldered into one of its fixtures is the impedance of that component together with its pads and their immediate surroundings, this trace geometry, this ground clearance, this set of neighbors, not some standalone property of the component alone. Solder the identical part onto a different PCB, with different trace dimensions, a different ground plane, and different neighboring components, and it will present a different effective inductance or capacitance and a different self-resonant frequency, the same lesson already drawn earlier in this document about the Testboard and about antennas. A fixture like this one is an excellent way to see how a component behaves and to compare parts against each other under identical, repeatable conditions. It is not a way to extract a single number that will carry over unchanged into a different design.

One practical caveat applies to readers of this document, which is written around a NanoVNA. Hellmich points out that true TRL needs a VNA with four receivers, able to measure the incident and scattered waves at both ports at the same time, and that it is therefore "not available on three receiver VNAs such as the venerable Hewlett-Packard HP 8753, and even less so on transmission-reflection VNAs such as HP 8752, and on virtually all ham radio or hobbyist kit." The NanoVNA is exactly that kind of transmission-reflection instrument: it measures S11 and S21 in one direction only, and its built-in calibration is the open, short, load and thru sequence used throughout this document, with no TRL option. So this board's TRL calibration is out of reach on a NanoVNA as it comes. What a NanoVNA can still do with a board like this, and this is this document's suggestion rather than anything Hellmich tried, is an approximate in-fixture version of its own open-short-load-thru routine. One clarification first: what this document has so far loosely called a two-port calibration is, on a transmission-reflection analyzer, a one-port correction at port 1 plus a normalized thru, the kind of correction Keysight calls an enhanced-response calibration. It is not the full two-port correction of an analyzer that can also drive port 2 and measure the part in reverse; that full two-port correction even the three-receiver HP 8753 just mentioned can do, and four receivers are what true TRL needs on top of it. The in-fixture trick then works like this: because Short and Match sit on the 15.5 mm offset, the Open within half a millimetre of it, and the Thru is exactly two of those offsets, treating them as ideal standards puts the port 1 reference plane on the 15.5 mm mark and, through the thru normalization, removes the matching 15.5 mm on the port 2 side, which is where the fixture pad edges are. Its reference impedance is then the Match's nominal 50 Ω, and the 15.5 mm of trace in front of each standard, whatever its impedance, is absorbed into the correction along with the cable, the jack and the launch, and with the analyzer's own imperfections, all of which the calibration lumps into a single fictitious two-port standing between the instrument and that plane, the error box. What limits it is elsewhere: none of the three standards is characterized (the Open has fringing capacitance, the Short a little inductance, the Match is two chip resistors with their own parasitics), each standard sits behind its own SMA jack and its own launch, so the calibration silently assumes that all of those launches, and the fixture's, behave identically (an assumption Hellmich's own TRL on this board shares, since its Thru, Line and Short also sit on different jacks), and a transmission-reflection analyzer, which never measures the part in reverse, has no way to correct for the load match of its own port 2 (that mismatch is easy enough to see, as S11 through the Thru after the one-port correction, but using it would need the DUT's S22 and S12). So the result degrades with frequency, but it should still be a large improvement over calibrating in the connector and measuring 15.5 mm of trace on each side of the part along with it, and his ferrite-bead comparison is exactly what makes the size of that error visible.

# Full-Wave 3D EM Simulation

Most of the prediction in this document has been the closed-form kind: a resonance formula for the band-stop branch, a pi-pad formula for the attenuators. Hellmich's own TRL board took the same approach at the design stage. In his words, "I have not run a sophisticated 3D EM simulation of the board but instead designed it according to the usual approximation formulas for coplanar waveguides, which should be acceptable at the frequencies involved." Those formulas assume an idealized geometry and a handful of material and geometry numbers, and they get checked afterward against whatever the analyzer actually measures on the real, soldered hardware. Full-wave 3D electromagnetic simulation is a different kind of prediction: instead of a formula standing in for the geometry, the actual 3D shape of the copper, the dielectric, and the connectors is built as a model, and a computer solves Maxwell's equations over that model directly.

Ansys HFSS is one of the best-known tools that do this, built primarily on the finite element method: it meshes the 3D model into small tetrahedral elements, solves the field equations on that mesh, and refines the mesh automatically wherever the solution error is largest, which in practice means wherever the field is changing fastest. Other solvers attack the same problem with different numerics: time-domain solvers on a structured grid (the finite integration technique in CST Studio Suite, a close relative of finite-difference time-domain, and finite-difference time-domain proper in the free, open-source openEMS), or method-of-moments solvers for planar circuits in Sonnet and in Keysight's ADS. The differences matter to specialists more than to a beginner; what they share is that the geometry is the input, not a formula. The umsakazo board itself was designed and checked the same hand-calculated, build-and-measure way as everything else in this document, not with full-wave simulation, but the alternative is worth knowing about, because it predicts the same quantities this document has spent so much effort measuring by hand: S-parameters, input impedance, and where a parasitic mode like the ones a via fence is built to suppress actually shows up, computed straight from the 3D model before any board is fabricated. It also reaches further than a bench VNA can on its own, into things like antenna radiation patterns and how one part of a system couples into another.

None of that replaces physical measurement and calibration; simulation complements them. A simulation is only as good as the material data behind it: dielectric properties, conductor losses, and the rest, the same kind of material sensitivity behind the earlier warning in this document that a fat, lossy solder joint can make a notch shallower. A real board also still carries manufacturing tolerances that no simulation sees in advance. Hellmich's coplanar line came out about 7% low in impedance, and its 45 mm between the reference planes reached 180° at 1.865 GHz instead of above 2 GHz. For the phase he offers only "some deviations of the board parameters from theory"; for the impedance, as reported earlier, he says the figure is within what a board process without impedance control delivers, and that it was measured through the connectors. Neither statement says which parameter is to blame. Some of that a field solver could have caught, because it models what the closed-form formulas leave out: a solder-mask layer over the trace and its 0.2 mm gaps, which adds capacitance and so pushes the impedance down and the electrical length up, exactly the direction this board went (at least with nominal mask data, since the real mask is not well characterized), and the launch itself, modelled as geometry instead of assumed away. It would also model the etched, trapezoidal profile of the 35 µm copper, although that one pushes the other way, raising the impedance rather than lowering it. The rest, the permittivity and thickness the fab actually delivered, no solver fed nominal values would see. What full-wave simulation buys is the chance to try out a geometry, catch a resonance landing where you don't want it, or size a ground clearance like the one shown earlier in this document, before committing to copper, rather than only finding out afterward with a soldering iron. Ansys HFSS (part of Synopsys since the acquisition of Ansys closed in July 2025, though it still ships under the Ansys name) is used this way for antennas, RF and microwave components, IC packages, and PCBs; Synopsys's own [RF design page](https://www.synopsys.com/solutions/rf-design.html), which describes its RF IC design flow, says that antennas, RF and microwave components and packages "are validated with Ansys HFSS full-wave 3D electromagnetic simulation for signoff-grade accuracy", and the tool itself is described in more detail on its [own product page](https://ansys.synopsys.com/products/electronics/ansys-hfss).
...
- Smith Chart by Veritasium: https://www.youtube.com/watch?v=GK2pZ_oVU1o
...
- Why Is Coax 50 Ohms? The Real Reason Explained by Telecom Training: https://www.youtube.com/watch?v=PkJfGCZl9cE
- The Smith Chart Made EASY! by TheSmokinApe Ham Radio: https://www.youtube.com/watch?v=_pgXQ02OcN4
- The TRUTH About High Feedpoint Reactance by TheSmokinApe Ham Radio: https://www.youtube.com/watch?v=_6C3If1R3Os 

## Doc

- What is a Smith Chart? An interactive introduction to the chart at the heart of RF engineering: https://www.arenaphysica.com/publications/smith-charts
...
- TRL calibration board with SMD test fixtures by Mario Hellmich: https://www.mariohellmich.de/projects/trl-cal/trl-cal.html
